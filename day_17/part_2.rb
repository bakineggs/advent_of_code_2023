...
until to_explore.empty? || best < to_explore.first[5]
  ...
  case dir
  when :right
    explore to_explore, row, col + 1, :right, steps, cost, min_cost unless col == max_col || steps == 10
    explore to_explore, row - 1, col, :up, 0, cost, min_cost unless row < 4 || steps < 4
    explore to_explore, row + 1, col, :down, 0, cost, min_cost unless row > max_row - 4 || steps < 4
  when :left
    explore to_explore, row, col - 1, :left, steps, cost, min_cost unless col == 0 || steps == 10
    explore to_explore, row - 1, col, :up, 0, cost, min_cost unless row < 4 || steps < 4
    explore to_explore, row + 1, col, :down, 0, cost, min_cost unless row > max_row - 4 || steps < 4
  when :down
    explore to_explore, row + 1, col, :down, steps, cost, min_cost unless row == max_row || steps == 10
    explore to_explore, row, col - 1, :left, 0, cost, min_cost unless col < 4 || steps < 4
    explore to_explore, row, col + 1, :right, 0, cost, min_cost unless col > max_col - 4 || steps < 4
  when :up
    explore to_explore, row - 1, col, :up, steps, cost, min_cost unless row == 0 || steps == 10
    explore to_explore, row, col - 1, :left, 0, cost, min_cost unless col < 4 || steps < 4
    explore to_explore, row, col + 1, :right, 0, cost, min_cost unless col > max_col - 4 || steps < 4
  end
end
